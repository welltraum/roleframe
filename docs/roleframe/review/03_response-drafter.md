...
| Тип | Файл | Роль |
|---|---|---|
| `prompt` | `evals/files/sample-agents/agents/response-drafter.md` | Control-слой с unsafe completion policy. |
| `runtime` | `evals/files/sample-agents/src/tools/factory.py` | Подтверждает, что риск идёт не от toolkit, а от prompt policy. |
| `doc` | `evals/expected-findings.md` | Фиксирует baseline finding про unsafe completion. |
...
- **Owner boundary**: Владеет только финальной сборкой ответа из подтверждённых данных. Не владеет поиском, routing и достраиванием фактов.

### Route matrix

| Source | Target | Status | Note |
|---|---|---|---|
| `supervisor` | `response-drafter` | implicit | Формат входа от supervisor не описан схемой. |
| `response-drafter` | `user` | implicit | Итоговый ответ уходит free text без strict schema. |

### Proof surfaces

- fixture review baseline для unsafe completion

### Deployment visibility

- visible только в конце orchestration contour
- нет signal на partial output

### Rollout

- нет gate на incomplete-input сценарии

### Preparedness

- unit unsafe-by-design, пока не введён honest partial path
...
| Input | 2 | Вход от других units понятен по роли, но не зафиксирован как structured handoff. | `evals/files/sample-agents/agents/response-drafter.md#L9-L12` |
