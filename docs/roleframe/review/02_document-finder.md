...
Document-finder уже близок к атомарной бизнес-функции. Главные пробелы сосредоточены не в boundary этого unit, а в handoff и failure policy. Пока результат годится для прототипа, но не для надёжного multi-unit контракта.

## Карта артефактов

| Тип | Файл | Роль |
|---|---|---|
| `prompt` | `evals/files/sample-agents/agents/document-finder.md` | Control и runtime hints для поиска документов. |
| `runtime` | `evals/files/sample-agents/src/tools/factory.py` | Показывает фактические capabilities toolkit. |
| `doc` | `evals/expected-findings.md` | Фиксирует ожидаемые проблемы по handoff и failure policy. |
...
- **Owner boundary**: Владеет только retrieval и bounded handoff документов. Не владеет final response или routing logic.

### Route matrix

| Source | Target | Status | Note |
|---|---|---|---|
| `supervisor` | `document-finder` | implicit | Входной payload не типизирован. |
| `document-finder` | `supervisor` | implicit | Возврат документов идёт prose-ответом. |

### Proof surfaces

- fixture review baseline
- tool factory cross-check

### Deployment visibility

- visible only through supervisor route
- нет явного bounded payload policy

### Rollout

- нет отдельного gate на timeout и retry policy

### Preparedness

- готов как retrieval prototype, не готов как typed reusable unit
...
| Input | 2 | Вход понятен по роли unit, но не оформлен как typed schema. | `evals/files/sample-agents/agents/document-finder.md#L9-L11` |
...
| Evaluation | 2 | Fixture review существует, но отдельные негативные сценарии unit не выделены. | `evals/evals.json#L225-L239`, `evals/expected-findings.md#L10-L14` |
...
| Safety | 2 | Unit не обещает лишнего, но tool-failure path всё ещё слаб. | `evals/files/sample-agents/agents/document-finder.md#L12-L14` |
...
- **Mechanism** · `evals/files/sample-agents/src/tools/factory.py#L1-L4` · Toolkit подтверждает, что у unit достаточно tools для функции поиска.
...
- [ ] Смоделировать timeout и убедиться, что unit возвращает status=error.
